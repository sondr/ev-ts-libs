...
export class FlatpickrAttr {
  ...
  valueChanged(date: Date, prevDate: Date) {
    ...
    console.log("setting date::", date, this.el._initialDate)
  ...
  //@bindable({ defaultBindingMode: bindingMode.oneTime }) options: Options;
  ...
  setDates(dates: Date[], currentDateString: string, instance: Instance, data?: any) {
    console.log("setting dates", dates, currentDateString);
    ...
    if (this.onUpdate) (PLATFORM.global as Window).setTimeout(() => { this.onUpdate({ value }); }, 0);
  }
  ...
  getDateStr(date: Date) {
    console.log("getDateStr", date, typeof (date));
    const self = this.el;
